...
function PostsList(){ // Destructuring props to get modal visibility state and close handler
    ...
    const [isFetching, setIsFetching] = useState(false);// State to manage the loading state of the posts

    useEffect(() => {
        async function fetchPosts() {// Asynchronous function to fetch posts data from the server
            setIsFetching(true);// Set the loading state to true
            ...
            setIsFetching(false);// Set the loading state to false
        }
